 ...
 * @version 2.4
 */
public class Processing_Receipts_OLD2
     ...
     */
    private Processing_Receipts_OLD2()
        {
    ...
    protected synchronized static void process_receipts(final Properties properties, final Gui gui) throws InterruptedException, java.lang.reflect.InvocationTargetException
        ...
        for (int i = 0; i < size; i++)
            ...
            // Kopieren des Quittungsfiles ins Archiv
            ...
            boolean exception_caused_by_copy_action = false;
            while (true)
                {
                try
                    {
                    ...
                    {
                    exception_caused_by_copy_action = true;
                    Logger.info("The receipt file "+file_receipt.getName()+" could not yet be copied from ");
                    Logger.info(file_receipt.getAbsolutePath()+" to ");
                    Logger.info(file_receipt_copy.getAbsolutePath()+".");
                    Logger.error(e);
                    }
                if (!files[i].exists())
                    {
            ...
            if (exception_caused_by_copy_action)
                {
                Logger.info("The receipt file "+file_receipt.getName()+" could finally be copied from ");
                Logger.info(file_receipt.getAbsolutePath()+" to ");
                Logger.info(file_receipt_copy.getAbsolutePath()+".");
                }
            ...
            Processing_Receipts_Record receipt = Processing_Receipts_ExtractElements_OLD2.extract(gui,file_receipt);
            
            RECEIPTS.add(receipt);
            
            //Receipt-File kann jetzt geloescht werden, da es verarbeitet worden ist.
            boolean exception_caused_by_delete_action = false;
            while (true)
                {
                try
                    {
                    ...
                    // Verschiebung der Quittung ins Archiv
                    Files.delete(file_receipt.toPath());
                    break;
                    }
                catch (Exception e)
                    {
                    exception_caused_by_delete_action = true;
                    Logger.info("The receipt file "+file_receipt.getAbsolutePath()+" could not yet be deleted.");
                    Logger.error(e);
                    }
                if (!files[i].exists())
                    {
            ...
            if (exception_caused_by_delete_action)
                {
                Logger.info("The receipt file "+file_receipt.getAbsolutePath()+" could finally be deleted.");
                }
